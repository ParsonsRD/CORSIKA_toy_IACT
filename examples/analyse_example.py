...
dataset = "CER000101.root" #"EPOS_10000GeV_proton.root"
...
# small chunk of code for getting avg&std values
...
if "proton" in dataset:
    ...
    energy_weight = np.repeat(header['energy'],9)
    bins = 100
    #plotting
    ...
    for i in range(0, len(length_division.ravel())):
        ...
        i += 1
    stdWidthapprox = np.reshape(stdWidthapprox, np.shape(width_division))
    stdLengthapprox = np.reshape(stdLengthapprox, np.shape(length_division))
...
# this part is just for 'bug searching'
    zerototen = []
    tentoonehundred = []
...
np.savetxt(textfile, header['energy'])
if "proton" in dataset:
    textfile.close()
else:
